...
void HostMonitor::ensureLogOpen() {
    ...
    // %APPDATA%\LANScopeFree\monitors (path is built below).
    wchar_t appData[MAX_PATH] = {};
    if (FAILED(::SHGetFolderPathW(nullptr, CSIDL_APPDATA, nullptr,
                                   SHGFP_TYPE_CURRENT, appData))) {
        return;
    }
    std::wstring dir = std::wstring(appData) + L"\\LANScopeFree\\monitors";
    // Create both levels (LANScopeFree and monitors) — ignore "already exists".
    ::CreateDirectoryW((std::wstring(appData) + L"\\LANScopeFree").c_str(), nullptr);
