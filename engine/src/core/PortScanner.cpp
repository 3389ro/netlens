    ...
    const OpenPortsCallback& onOpenPorts)
{
    ...
    for (size_t batchStart = 0; batchStart < ports.size(); batchStart += kBatchSize) {
        ...
        size_t probeCount    = 0;
        size_t probesAttempted = 0;   // ports that produced an actual probe outcome
        ...
        // Count only ports we actually probed (skipping invalid port
        // numbers and socket-creation failures). The caller's progress
        // should reflect real work, not skipped work — otherwise the
        // total can exceed the estimate and show >100% in the UI.
        if (probesDone && probesAttempted > 0) {
            probesDone->fetch_add(static_cast<int64_t>(probesAttempted),
